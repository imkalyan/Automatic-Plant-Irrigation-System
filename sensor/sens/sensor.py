...
class sensor:
    ...
    def startmotor(self):
        GPIO.output(self.motorgpiopin, True)
        #time.sleep(3)
        ##GPIO.output(self.motorgpiopin,False)
    def stopmotor(self):
        GPIO.output(self.motorgpiopin,False)
    def calculatemoisture(self):
        soil_moisture = mcp3008.readadc(self.moisturepin)        #taking soilmoisture value from adc pin 5
    ...
    def mailmessage(self):
        ...
        '''try:
            ...
            print 'no message received from raspberry pi'''
...
while True:
    ...
    if(watersensor_reading>=15):
        ...
        print("not Raining")
        k=1
    distance = plant1.calculate_tankheight()
    print "------------------------------------------------"
